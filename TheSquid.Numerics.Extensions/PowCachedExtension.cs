...
{
    /// <summary>
    /// Cache with format: basement, exponent, power, counter. 
    /// </summary>
    ...
    public static BigInteger PowCached(this ref BigInteger basement, int exponent)
    {
        try
        {
            const string negativeExponentMessage = "The value of the exponent parameter is negative.";
            if (exponent < 0) throw new ArgumentOutOfRangeException(nameof(exponent), negativeExponentMessage);
